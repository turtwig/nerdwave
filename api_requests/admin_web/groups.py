...
class AssociateGroupAddAlbum(api.web.APIHandler):
    ...
    sid_required = False
    fields = {
        "album_id": (api.fieldtypes.album_id, True),
        "album_sid": (api.fieldtypes.sid, True),
    }
...
class AssociateGroupToolFinish(api.web.HTMLRequest):
    ...
    def get(self, group_id):
        ...
        self.write(
            "<p>Now associated.</p><p><a href='/admin/tools/associate_groups'>Start over.</a></p>"
        )
...
class AssociateGroupCacheReset(api.web.HTMLRequest):
    ...
    def get(self):
        ...
        self.write(self.render_string("bare_header.html", title="Added Groups"))
        self.write(
            "<p>Reset.</p><p><a href='/admin/tools/associate_groups'>Start over.</a></p>"
        )
        self.write(self.render_string("basic_footer.html"))
...
class AssociateGroupTool(api.web.HTMLRequest):
    ...
    def get(self):
        ...
        for row in db.c.fetch_all(
            ...
            self.write(
                "<option value='%s'>%s</option>" % (row["group_id"], row["group_name"])
            )
        ...
        self.write(
            "<p><input type='text' id='new_group_name' /><br /><button onclick=\"window.top.call_api('admin/create_group', { 'name': document.getElementById('new_group_name').value })\">Create</button></p>"
        )
        self.write(self.render_string("basic_footer.html"))
...
class AssociateGroupAlbumList(AlbumList):
    def render_row_special(self, row):
        self.write(
            "<td><a onclick=\"window.top.call_api('admin/associate_groups_add_album', { 'album_id': %s, 'album_sid': window.top.current_restriction });\">Add To List</a>"
            % row["id"]
        )
...
class AssociateGroupSongList(SongList):
    def render_row_special(self, row):
        self.write(
            "<td><a onclick=\"window.top.call_api('admin/associate_groups_add_song', { 'song_id': %s });\">Add to List</a></td>"
            % row["id"]
        )
        self.write("<td>")
        for group in db.c.fetch_all(
            ...
            if not group["group_is_tag"]:
                self.write(
                    "<a class='group_name group_delete' onclick=\"window.top.call_api('admin/remove_group_from_song', { 'song_id': %s, 'group_id': %s });\">%s (X)</a> "
                    % (row["id"], group["group_id"], group["group_name"])
                )
            else:
                self.write(
                    "<span class='group_name'>%s</span>" % (group["group_name"],)
                )
        self.write("</td>")
...
class GroupEditTool(api.web.HTMLRequest):
    ...
    def get(self):
        ...
        self.write(
            "<p>The station you select in the frames above this have no bearing here.</p>"
        )
        self.write(self.render_string("basic_footer.html"))
...
class GroupEditGroupList(api.web.HTMLRequest):
    ...
    def get(self):
        ...
        for row in groups:
            ...
            self.write(
                "<td onclick=\"window.location.href = '../song_list/' + window.top.current_tool + '?id=%s';\" style='cursor: pointer;'>%s</td><td>"
                % (row["id"], row["name"])
            )
            ...
            if row["cool_time"] == None:
                row["cool_time"] = ""
            self.write(
                "<td><input type='text' id='elec_block_%s' value='%s' /><button onclick=\"window.top.call_api('admin/edit_group_elec_block', { 'group_id': %s, 'elec_block': document.getElementById('elec_block_%s').value })\">BLK</button></td>"
                % (row["id"], row["elec_block"], row["id"], row["id"])
            )
            self.write(
                "<td><input type='text' id='cooldown_%s' value='%s' /><button onclick=\"window.top.call_api('admin/edit_group_cooldown', { 'group_id': %s, 'cooldown': document.getElementById('cooldown_%s').value })\">CD</button></td>"
                % (row["id"], row["cool_time"], row["id"], row["id"])
            )
...
class GroupEditSongList(api.web.HTMLRequest):
    ...
    def get(self):
        ...
        for row in db.c.fetch_all(
            ...
            self.write(
                "<tr><td>%s</th><td>%s</td><td>" % (row["id"], row["album_name"])
            )
            self.write("</td><td>%s</td><td>" % row["title"])
            if not row["group_is_tag"]:
                self.write(
                    "<a class='group_name group_delete' onclick=\"window.top.call_api('admin/remove_group_from_song', { 'song_id': %s, 'group_id': %s });\">%s (X)</a> "
                    % (row["id"], group.id, group.data["name"])
                )
            self.write("</td></tr>")
        self.write(self.render_string("basic_footer.html"))
