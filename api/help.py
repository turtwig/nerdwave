...
help_classes: dict[
    str,
    api.web.NerdwaveHandler | api.web.APIHandler,
] = {}
url_properties = (
...
class IndexRequest(api.web.HTMLRequest):
    ...
    def get(self):
        self.write(
            self.render_string(
                "basic_header.html", request=self, title="Nerdwave API Documentation"
            )
        )
...
<pre>xhr.open("POST", "https://nerdwave.cc/api4/album", true);
xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<pre>xhr.open("POST", "https://nerdwave.cc/api4/playback_history", true);
xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<p>Please experiment by making requests yourself and looking at the output.  Parameters and conditions are noted in the table above.</p>"""
        ...
        self.write(
            "<ul><li>The Nerdwave API endpoints are all: <b>https://nerdwave.cc/api4/[URL]</b>, with URL corresponding to the table above.</li>"
        )
        self.write(
        ...
        self.write(
            "<li>Authentication keys and user IDs are only visible to end users, they can see theirs at <a href='https://nerdwave.cc/keys/'>https://nerdwave.cc/keys/</a></li>"
        )
        self.write(
            "<li>If an API Key is required in the table above and the user is anonymous, you will not be able use that functionality.  API keys for anonymous users are only given to those using the nerdwave.cc site and expire quickly.  Registered user's API keys do not expire.</li>"
        )
        self.write(
            "<li>Desired station for the request must be specified by 'sid' form data, using the corresponding Station ID below.</li>"
        )
        self.write(
            "<li>Nerdwave's API is complete, and will not be changing.  New functionality will only be done via new commands.</li>"
        )
        self.write(
...
class HelpRequest(tornado.web.RequestHandler):
    def get(self, url):
        ...
        self.write(
            self.render_string("basic_header.html", title="Nerdwave API - %s" % url)
        )
        ...
        for prop in url_properties:
            if prop[0] == "auth_required" and getattr(cls, "phpbb_auth", False):
                self.write(
                    "<li>User must be requesting from Nerdwave.cc and logged in to the Nerdwave.cc forum system."
                )
