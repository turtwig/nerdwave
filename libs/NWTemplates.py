...
def compile_templates(source_dir, dest_file, **kwargs):
    ...
    o.write(
        js_start(
            "full_calls" in kwargs and kwargs["full_calls"],
            "helpers" in kwargs and kwargs["helpers"],
        )
    )
    for root, _subdirs, files in os.walk(source_dir):
        for f in files:
            if f.endswith(".hbar") or f.endswith(".html"):
                tname = os.path.join(
                    root[root.find(source_dir) + len(source_dir) + len(os.sep) :],
                    f[: f.rfind(".")],
                ).replace(os.sep, ".")
...
raw_js_functions = ["$l", "gettext", "interpolate"]

_bind_reserved_words = (
    "update",
    "get",
    "update_data",
    "clear",
...
class NerdwaveParser(HTMLParser):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not template_name in inline_templates:
            ...
            self.buffers["_r"] += (
                "if (!_r){_r=%s.createDocumentFragment();" % self.calls.document
            )
            ...
            if not full_calls:
                self.buffers["_r"] += "_r.%s=_r.%s;" % (
                    _short_call.appendChild,
                    _full_call.appendChild,
                )
            self.buffers["_r"] += "}"
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "svg":
            ...
            if svg_use:
                self.buffers[self._current_stack_point()] += "var %s=_svg(%s);" % (
                    uid,
                    svg_use,
                )
            else:
            ...
            self.buffers[self._current_stack_point()] += "var %s=%s.%s('%s');" % (
                uid,
                self.calls.document,
                self.calls.createElement,
                tag,
            )
        ...
        for attr in attrs:
            ...
            if attr[0] == "bind":
                ...
                self.buffers[self._current_stack_point()] += "%s.className=%s;" % (
                    uid,
                    attr_val,
                )
            elif attr[0] == "href" and tag != "svg":
                self.buffers[self._current_stack_point()] += "%s.href=%s;" % (
                    uid,
                    attr_val,
                )
                ...
                if not self.stack[-1]["function_id"] in self.bound_scopes:
                    ...
                    if not self.helpers_on:
                        ...
                        self.buffers[self._current_stack_point()] += (
                            "if(!_c.$t){_c.$t=new _o(_c);_c.$t._root=%s;}"
    ...
    def handle_bind(self, uid, bind_name):
        ...
        if not self.helpers_on:
            self.buffers[self._current_stack_point()] += "_c.$t.%s=%s;" % (
                bind_name,
                uid,
            )
        else:
            ...
            if bind_name != "item_root":
                self.buffers[
                    self._current_stack_point()
                ] += "_h.elem_update(%s, _c.%s);" % (uid, bind_name)

    def handle_append(self, uid):
        self.buffers[self._current_stack_point()] += "%s.%s(%s);" % (
            self._current_tree_point(),
            self.calls.appendChild,
            uid,
        )
    ...
    def handle_endtag(self, tag):
        ...
        try:
            ...
            if tag != tn_tag:
                raise Exception(
                    "%s has mismatched open/close tags. (%s - %s%s)"
                    % (self.name, tag, tn_tag, attrs)
                )
        ...
        for line in self.input_buffer.split("\n"):
            for data in re.split(r"({{.*?}})", line):
                ...
                if not data or len(data) == 0:
                    ...
                    self.buffers[self._current_stack_point()] += "%s.textContent" % (
                        self.tree[-1],
                    )
                    ...
                    if self.tree[-1] in self.helpers:
                        self.buffers[self._current_stack_point()] += "_h[%s](%s);" % (
                            self.helpers[self.tree[-1]],
                            self._parse_val(data),
                        )
                    else:
    ...
    def handle_stack_push(self, data):
        ...
        if (
            ...
            self.stack.append(entry)
            self.buffers[self._current_stack_point()] = "%s=function%s(_c, %s){" % (
                entry["function_id"],
                dbg_name,
                self._current_tree_point(),
            )
    ...
    def handle_stack_pop(self, name):
        ...
        if not name == self.stack[-1]["name"] and not (
            name == "if" and self.stack[-1]["name"] == "else"
        ):
            raise Exception(
                "Mismatched open and close tags (%s and %s) in %s template."
                % (self.stack[-1]["name"], name, self.name)
            )
        ...
        if not self.stack:
            ...
            self.scope_bound = False
        getattr(self, "handle_%s" % stackpt["name"])(
            stackpt["function_id"], stackpt["argument"]
        )
    ...
    def handle_subtemplate(self, template_name):
        ...
        self.buffers[self._current_stack_point()] += "_rwt.%s(_c, %s);" % (
            template_name,
            self._current_tree_point(),
        )
    ...
    def handle_else(self, function_id, context_key):
        context_key = self.parse_context_key(context_key)
        self.buffers[self._current_stack_point()] += "if(!(%s))%s(_c, %s);" % (
            context_key,
            function_id,
            self._current_tree_point(),
        )
...
if __name__ == "__main__":
    ...
    compile_templates(
        ...
        full_calls=command_args.full,
        helpers=command_args.helpers,
    )
