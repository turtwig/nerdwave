...
BOOTSTRAP.on_init.push(function DJPanel(root_template) {
	...
	javascriptNode.onaudioprocess = function(evt) {
		...
		if (peak > javascriptNode.longPeak) {
			...
			javascriptNode.longPeak = Math.max(
				analyser.minDecibels,
				javascriptNode.longPeak - (1 - javascriptNode.averaging) * Math.abs(javascriptNode.longPeak)
			);
		}

		var rms = Math.sqrt(sum / bufLength);
		javascriptNode.rms = Math.max(
			rms,
			Math.min(analyser.minDecibels, javascriptNode.rms * javascriptNode.averaging)
		);
	...
	t.btn_on_air.addEventListener("click", function() {
		...
		var ws_url =
			"ws://rwdj:" +
			dj_api_status.dj_password +
			"@" +
			dj_api_status.mount_host +
			":" +
			dj_api_status.mount_port +
			"/" +
			dj_api_status.mount_url;
