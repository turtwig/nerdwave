var Timeline = (function () {
  ...
  INIT_TASKS.on_init.push(function (root_tmpl) {
    ...
    if (
      visibilityEventNames &&
      visibilityEventNames.change &&
      document.addEventListener
    ) {
      document.addEventListener(
        visibilityEventNames.change,
        live_voting_visibility_change,
        false
      );
    }
  ...
  self.update = function () {
    ...
    for (i = sched_history.length - 1; i >= 0; i--) {
      ...
      if (sched_history[i].el.parentNode != el) {
        sched_history[i].$t.progress.parentNode.removeChild(
          sched_history[i].$t.progress
        );
      }
    ...
    for (i = 0; i < sched_next.length; i++) {
      ...
      if (
        previous_evt &&
        sched_next[i].core_event_id &&
        previous_evt.core_event_id === sched_next[i].core_event_id
      ) {
        is_continuing = true;
    ...
    for (i = 0; i < events.length; i++) {
      if (events[i]._pending_delete) {
        events[i].el.style[Fx.transform] =
          "translateY(-" +
          (Sizing.song_size_np +
            (Sizing.song_size * events[i].songs.length - 1)) +
          "px)";
  ...
  self.reflow = function (reflow_everything) {
    ...
    for (i = 0; i < messages.length; i++) {
      if (!messages[i].closed) {
        messages[i].$t.el.style[Fx.transform] =
          "translateY(" + running_y + "px)";
        running_y += Sizing.timeline_message_size + 5;
      }
    }

    template.history_header.style[Fx.transform] =
      "translateY(" + running_y + "px)";

    var history_size = Prefs.get("l_stk")
      ? sched_history.length
      : Prefs.get("l_stksz") || 0;
    ...
    var hidden_events = Math.min(
      sched_history.length,
      Math.max(0, sched_history.length - history_size)
    );
    for (i = 0; i < hidden_events && i < sched_history.length; i++) {
      events[i].el.style[Fx.transform] =
        "translateY(" +
        -(((hidden_events - i - 1) * 5 + 1) * Sizing.song_size + 1) +
        "px)";
    ...
    for (i = hidden_events; i < events.length; i++) {
      if (events[i].history) {
        ...
        history_gap = true;
        history_bar.style[Fx.transform] =
          "translateY(" + (running_y + 9) + "px)";
        running_y += 19;
      }
  ...
  self.vote = function (which_election, song_position) {
    ...
    if (
      song_position < 0 ||
      song_position > sched_next[which_election].songs.length
    ) {
      throw { is_rw: true, tl_key: "invalid_hotkey_vote" };
    }
  ...
  self.voting_allowed_check = function () {
    ...
    for (var i = 0; i < sched_next.length; i++) {
      if (!sched_next[i].disable_voting) {
        ...
        sched_next[i].type != "election" ||
        sched_next[i].songs.length <= 1
      ) {
  ...
  var lock_check = function (json) {
    if (json.lock_in_effect && json.lock_sid != json.sid) {
      ...
      self.add_message(
        "station_lock",
        $l("locked_to_station", {
          locked_station: locked_name,
          this_station: this_name,
