  ...
  INIT_TASKS.on_draw.push(function (root_template) {
    ...
    scroll.reposition_hook = function () {
      if (current_type && current_id)
        scroll_positions[current_type][current_id] = scroll.scroll_top;
    };
  });
  ...
  var actually_open = function (typ, id) {
    ...
    if (!cache[typ][id]) {
      ...
      if (
        t._root &&
        t._root.tagName &&
        t._root.tagName.toLowerCase() == "div"
      ) {
        cache[typ][id] = t;
  ...
  self.open_route = function (typ, id) {
    ...
    if (typ in lists && id && !isNaN(id)) {
      ...
      if (!ready_to_render) {
        remove_excess_header_content();
        detail_header.textContent =
          (lists[typ] && lists[typ].get_title_from_id
            ? lists[typ].get_title_from_id(id)
            : false) || $l("Loading...");
      }
      var scrolled = false;
