var SongList = function () {
  ...
  self.reflow = function () {
    ...
    for (var i = 0; i < songs.length; i++) {
      ...
      songs[i].el.style[Fx.transform] =
        "translateY(" + (running_height + Sizing.height * (i + 1)) + "px)";
  ...
  self.real_reflow = function () {
    ...
    padder.style[Fx.transform] =
      "translateY(" + (running_height - Sizing.request_size) + "px)";
    scroller.set_height(running_height);
  };
  ...
  var continue_drag = function () {
    ...
    var new_y =
      original_request_y -
      (original_mouse_y - (last_mouse_event.clientY + scroller.scroll_top));
    if (new_y != current_dragging_y) {
      current_dragging_y = new_y;
      var new_index = Math.floor(
        (new_y + Sizing.request_size * 0.3) / Sizing.request_size
      );
    ...
    if (last_mouse_event.clientY < upper_fold && scroller.scroll_top > 0) {
      scroller.scroll_to(
        scroller.scroll_top -
          (25 - Math.floor((last_mouse_event.clientY / upper_fold) * 25))
      );
    } else if (
      last_mouse_event.clientY > Sizing.height - lower_fold &&
      scroller.scroll_top < scroller.scroll_top_max
    ) {
      scroller.scroll_to(
        scroller.scroll_top +
          Math.floor(
            ((lower_fold - (Sizing.height - last_mouse_event.clientY)) /
              lower_fold) *
              20
          )
      );
      ...
      last_mouse_event.clientY > upper_normal_fold + 30
    ) {
      upper_fold = upper_normal_fold;
    } else if (
      lower_fold != lower_normal_fold &&
      last_mouse_event.clientY < Sizing.height - lower_normal_fold - 30
    ) {
      lower_fold = lower_normal_fold;
    }
...
var Requests = (function () {
    ...
    if (User.tuned_in) {
      if (!User.requests_paused) {
        if (link && good_requests) {
          if (!Sizing.simple) {
            link_text.textContent = $l("#_requests", {
  ...
  self.remove = function (song_id) {
    ...
    API.async_get("delete_request", { song_id: song_id }, null, function () {
  ...
  var clicked = function () {
    ...
    if (User.id === 1) {
      ErrorHandler.tooltip_error(
        ErrorHandler.make_error("must_login_and_tune_in_to_request", 400)
      );
    }
