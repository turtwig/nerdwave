var NerdwaveAPI = (function () {
  ...
  var initSocket = function () {
    ...
    var wshost =
      window.location.protocol === "https:" || self.forceSecure
        ? "wss://"
        : "ws://";
    if (_host) {
      wshost += _host;
    ...
    socket.addEventListener("open", function () {
      ...
      try {
        ...
        console.error(
          "Socket exception while trying to authenticate on socket."
        );
        console.error(exc);
        onSocketError();
      }
  ...
  var onSocketOK = function () {
    ...
    if (currentScheduleID) {
      if (self.debug)
        console.log(
          "Socket send - check_sched_current_id with " + currentScheduleID
        );
      try {
        socket.send(
          JSON.stringify({
        ...
        console.error(
          "Socket exception while trying to check_sched_current_id."
        );
        console.error(exc);
        onSocketError();
      }
    ...
    if (asyncRequest) {
      for (i in json) {
        if ("success" in json[i] && !json[i].success) {
          ...
          if (
            self.throwErrorsOnThrottle ||
            json[i].tl_key !== "websocket_throttle"
          ) {
    ...
    if (statelessRequests.indexOf(action) !== -1 || !isOK) {
      for (var i = requestQueue.length - 1; i >= 0; i--) {
        if (requestQueue[i].message.action === action) {
          if (self.debug)
            console.log(
              "Throwing away extra " + requestQueue[i].message.action
            );
