var Rating = (function () {
    ...
    for (i = 0; i < ratings.length; i++) {
      if (json.rating_user) {
        ...
        if (ratings[i].lastChild) {
          ratings[i].lastChild.textContent = Formatting.rating(
            json.rating_user
          );
        }
        if (
  ...
  var get_rating_from_mouse = function (evt) {
    var x, y;
    if (
      typeof evt.offsetX != "undefined" &&
      typeof evt.offsetY != "undefined"
    ) {
      x = evt.offsetX;
      y = evt.offsetY;
    } else {
  ...
  var scroll_check = function (e) {
    if (
      e.touches[0].pageY < scroll_check_min ||
      e.touches[0].pageY > scroll_check_max
    ) {
      touchend(e);
    }
    last_touch = e;
  ...
  var do_touch_rating = function () {
    ...
    var remove = function (e) {
      if (
        ...
        touching_song &&
        touching_song.$t &&
        e.target == touching_song.$t.rating
      ) {
    ...
    var highlight = function (rating, width) {
      ...
      t.slider.style.backgroundPosition =
        ...
        -(
          Math.max(
            5,
            Math.min(25, Math.floor(width / ((slider_width - 25) / 24)))
          ) * 96
        ) +
        "px";
    ...
    var touchmove = function (e) {
      ...
      var rating =
        Math.floor(
          (e.touches[0].pageX - (zero_x + 25)) / ((slider_width - 25) / 9)
        ) /
          2 +
        1;
      ...
      highlight(
        rating,
        Math.min(slider_width, Math.max(0, e.touches[0].pageX - zero_x))
      );
    };
  ...
  self.do_rating = function (new_rating, json) {
    ...
    confirm.style[Fx.transform] =
      "translateX(" +
      Math.round((new_rating / 5.0) * 50 - 15) +
      "px) scaleX(0.2)";
    ...
    API.async_get(
      "rate",
      { rating: new_rating, song_id: json.id },
      function (newjson) {
        requestNextAnimationFrame(function () {
  ...
  self.register = function (json) {
    ...
    if (!Sizing.simple && json.rating_user) {
      json.$t.rating_hover_number.textContent = Formatting.rating(
        json.rating_user
      );
    }
